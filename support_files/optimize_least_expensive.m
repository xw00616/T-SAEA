function [Pop,Fitness] = optimize_least_expensive(Population,Bounds,latency,Problem,M,id_nex)
    
    FE_Max = size(Population,1)*(latency-1);
    
    Archive = call_GA(Population,FE_Max,Bounds,Problem,M,id_nex);
    
    Pop = Archive(:,1:end-1);
    Fitness = Archive(:,end);
end

function Archive = call_GA(Population,FE_Max,Boundary,Problem,M,id_nex)
    ...
    while FE <FE_Max
        ...
        F_Values = P_objective1('value', Problem, M, Offspring);
